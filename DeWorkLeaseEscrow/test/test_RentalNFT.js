...
describe("Test RentalNFT", function () {
  let owner, owner1, owner2, user1, user2;

  before(async () => {
    [owner, owner1, owner2, user1, user2] = await ethers.getSigners();
  ...
  it("Should set user for the first NFT", async function () {
    const expires = Math.floor(Date.now() / 1000) + 3600; // 1 hour from now
    await rentalNFT.connect(owner1).setUser(1, user1.address, expires);
    expect(await rentalNFT.userOf(1)).to.equal(user1.address);
  })

  it("Should check expires for the first NFT", async function () {
    const expires = await rentalNFT.userExpires(1);
